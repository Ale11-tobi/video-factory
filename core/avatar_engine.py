...
class AvatarEngine:
    ...
    def generate_segment(self, start_sec: float, duration: float, segment_id: str) -> str:
        """
        Genera una singola clip video dell'Avatar parlante per una specifica porzione audio.
        Ritorna il percorso del video generato.
        """
        import subprocess
        import glob
        import shutil
        ...
        BASE_DIR = os.path.dirname(os.path.dirname(__file__))
        sadtalker_dir = os.path.join(BASE_DIR, "SadTalker")
        segment_audio = os.path.join(self.temp_path, f"segment_audio_{segment_id}.wav")
        segment_video = os.path.join(self.temp_path, f"avatar_segment_{segment_id}.mp4")
        base_video = os.path.join(self.temp_path, f"avatar_base_{segment_id}.mp4")

        # 1. Estrai l'audio per questo segmento esatto
        subprocess.run([ffmpeg_exe, "-y", "-i", self.master_voice, "-ss", str(start_sec), "-t", str(duration), segment_audio], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

        # 2. Crea il Ken Burns di base come fallback garantito
        self._fallback_render(ffmpeg_exe, segment_audio, base_video)

        if os.path.exists(sadtalker_dir):
            logger.info(f"Avvio SadTalker per segmento dinamico {segment_id} ({duration}s)...")
            out_dir = os.path.join(self.temp_path, f"sadtalker_out_{segment_id}")
            os.makedirs(out_dir, exist_ok=True)
            
            inference_script = os.path.join(sadtalker_dir, "inference.py")
            cmd = [
                python_exe, inference_script,
                "--driven_audio", segment_audio,
                "--source_image", self.source_image,
                "--result_dir", out_dir,
                "--still", "--preprocess", "full"
            ]
            ...
            try:
                ...
                generated_files = glob.glob(os.path.join(out_dir, "*.mp4"))
                if generated_files:
                    shutil.move(generated_files[0], segment_video)
                    logger.info(f"Segmento SadTalker completato: {segment_video}")
                    return segment_video
                else:
                    logger.warning("SadTalker non ha prodotto output per il segmento. Uso fallback.")
            except Exception as e:
                logger.error(f"Errore SadTalker segmento {segment_id}: {e}. Uso fallback.")
        
        # Fallback (SadTalker assente o in errore)
